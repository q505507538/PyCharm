...
def get_rss(content):
    ...
    for x in xrange(1, len(soup.contents)):
        ...
        rss = PyRSS2Gen.RSSItem(
            ...
            description=get_content(soup.contents[x].find_all('h2')[0].contents[0].get('href'))
        )
...
def get_content(url):
    ...
    content = split(html, '<article class="article-content">', '</article>')
    if content <> -1:
        return '<article class="article-content">' + content + '</article>'
    else:
        print "获取正文内容失败"
        sys.exit(0)
...
def test_content(url):
    ...
    content = split(html, '<article class="article-content">', '</article>')
    if content <> -1:
        print '<article class="article-content">' + content + '</article>'
    else:
        print "获取正文内容失败"
